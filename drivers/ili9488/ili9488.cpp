...
#define writecommand LCD_WriteReg
#define writedata LCD_WriteData
...
//#define EXAMPLE_SETUP
    ...
    // GramScanWay depends on rotation : display scan and color transfer mode
    LCD_WriteReg(0xB6); // DFUNCTR
		LCD_WriteData(0x00);
		LCD_WriteData(0x62);
  ...
  void ILI9488::configure_display(Rotation rotate) {
  ...
  void ILI9488::write_blocking_dma(const uint8_t *src, size_t len) {
  ...
  void ILI9488::partial_update(PicoGraphics *display, Rect region) {
    // check sanity flag
    if(in_dma_update) {
  ...
  void ILI9488::update(PicoGraphics *graphics) {
    // check sanity flag
    if(in_dma_update) {
      panic("When use_async_dma is set you must call wait_for_update_to_finish() between updates");
    }
  ...
  void ILI9488::set_backlight(uint8_t brightness) {
  ...
  void ILI9488::setup_dma_control_if_needed() {
    if(use_async_dma) {
  ...
  void ILI9488::enable_dma_control(bool enable) {
    if(use_async_dma) {
  ...
  void ILI9488::start_dma_control() {
  ...
  bool ILI9488::set_update_region(Rect& update_region){
  ...
  void ILI9488::set_addr_window(uint16_t x, uint16_t y, uint16_t w, uint16_t h)	{
